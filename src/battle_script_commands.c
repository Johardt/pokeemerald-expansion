...
static u32 GetTrainerMoneyToGive(u16 trainerId)
{
    ...
    else
    {
        ...
        if (gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS)
            moneyReward = 20 * lastMonLevel * gBattleStruct->moneyMultiplier * trainerMoney;
        else if (IsDoubleBattle())
            moneyReward = 20 * lastMonLevel * gBattleStruct->moneyMultiplier * 2 * trainerMoney;
        else
            moneyReward = 20 * lastMonLevel * gBattleStruct->moneyMultiplier * trainerMoney;
    }
